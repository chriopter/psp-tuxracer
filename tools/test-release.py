...
class ReleaseTests(unittest.TestCase):
    ...
    def test_unrelated_published_tag_after_svn_import(self):
        with tempfile.TemporaryDirectory() as directory:
            old = os.getcwd()
            os.chdir(directory)
            try:
                ...
                git('config', 'user.name', 'CI test')
                git('config', 'user.email', 'ci@example.invalid')
                git('commit', '--allow-empty', '-qm', 'Legacy PSP history')
                git('tag', 'v0.35.0')
                git('checkout', '--orphan', 'fork')
                git('commit', '--allow-empty', '-qm', 'Original SVN release')
                base = git('rev-parse', 'HEAD')
                Path('docs').mkdir()
                Path('docs/upstream.json').write_text(json.dumps({'git_base': base}))
                git('add', 'docs/upstream.json')
                git('commit', '-qm', 'Port Extreme Tux Racer to PSP')
                ...
                git('tag', 'v0.36.0', fork)
                git('commit', '--allow-empty', '-qm', 'Next PSP improvement')
                result = publish.notes('HEAD', 'v0.36.0')
