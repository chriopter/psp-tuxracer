...
cd "$project_dir"
game_dir="$XDG_CONFIG_HOME/ppsspp/PSP/GAME/ExtremeTuxRacer"
built_game="$project_dir/ports/extremetuxracer/psp/EBOOT.PBP"
# A bare build EBOOT has no adjacent game data. Refresh the complete staged
# game after a rebuild, retaining the user's configuration and saved games.
if [[ -f "$built_game" ]] && {
    ! cmp -s "$built_game" "$game_dir/EBOOT.PBP" ||
    [[ ! -f "$game_dir/data/textures/textures.lst" ||
       ! -f "$game_dir/data/translations/languages.lst" ]];
}; then
    python3 "$project_dir/tools/stage-extremetuxracer.py"
fi
if [[ ! -f "$game_dir/EBOOT.PBP" ||
      ! -f "$game_dir/data/textures/textures.lst" ||
      ! -f "$game_dir/data/translations/languages.lst" ]]; then
    echo 'Build the game with ./build-extremetuxracer.sh before starting it.' >&2
    exit 1
fi
...
exec "$emulator_bin" --windowed --log="$project_dir/logs/etr-ppsspp.log" "$game_dir/EBOOT.PBP"
