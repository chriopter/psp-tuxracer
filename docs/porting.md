...
Das Hauptmenü behält die ursprüngliche Anordnung mit großem Logo, Eisrändern und fallendem Schnee. Die GUI-Schneeflocken werden gemeinsam gezeichnet statt einzeln. Nach der Spielerauswahl erscheint vor dem ersten Hauptmenübesuch eine per Bildmodell erzeugte PSP-Illustration (512 × 256, RGB565, 256 KiB) mit nativ gezeichneten Beschriftungen und Verbindungslinien. Start öffnet das Menü; Help zeigt die Grafik erneut. Der gemeinsame Button-Atlas (256 × 128, 64 KiB) bleibt für die Hinweise und Bestätigungsdialoge erhalten; direkt gezeichnete Symbole dienen als Fallback.

Kreuz bestätigt, Kreis geht zurück; Start pausiert im Rennen bzw. setzt eine Pause fort. Nur Einführung und Help verwenden Start zusätzlich zum Weitergehen. Schulter-/Trick-/Reset-Tasten bleiben in Menüs ohne Aktion. Rennabbruch und Spielende benötigen eine zweite Bestätigung, Hilfe/Credits schließen nicht durch Richtungstasten. `tools/test-psp-controls.py` kompiliert die tatsächliche Belegung aus dem Plattformcode und prüft 55 Zuordnungen sowie gehaltene und gleichzeitige Tasten. `tools/test-psp-ui.py` prüft die tatsächliche Schnee-Geometrie und den Start-Einführungsdialog. Screenshots, Messwerte und Asset-Herkunft stehen in [der UI-Validierung](psp-ui-validation.md).
...
- Terrain-Dreiecke am Sichtfeldrand werden vor der Perspektivprojektion zugeschnitten. Das verhindert, dass die PSP-Geometrieeinheit ganze Dreiecke außerhalb ihres Koordinatenbereichs verwirft; UVs, Normalen und Misch-Alpha werden interpoliert. Der Hosttest prüft den tatsächlichen Renderer einschließlich 10.000 zufälliger Grenzfälle.
- Tacho und Sprungkraftanzeige verwenden explizite UVs statt des von der PSP-Anbindung nicht unterstützten Desktop-Texgen.
- Auch Skybox-Flächen werden vor der Projektion zugeschnitten, damit beim Lenken keine ganzen Himmelsflächen verschwinden. Windpfeile nutzen Float-Koordinaten wie die übrige HUD-Geometrie.
...
- Fallender Schnee wird pro Schneeschicht in einem Vertex-Batch gezeichnet statt mit bis zu 3.000 einzelnen Matrix-/Zeichenaufrufen pro Frame.
